...
import type { PalmConsultationTicket } from '@/components/consultation/consultation-queue';

// Global mock storage for pending/completed consultation tickets in Next.js memory
let consultationTickets: PalmConsultationTicket[] = [
  {
    id: 't-101',
...
export async function GET() {
  const pendingTickets = consultationTickets.filter((t) => t.status === 'Pending');
  return NextResponse.json({ tickets: pendingTickets });
}

export function updateTicketInStore(ticketId: string, notes: string) {
  const ticket = consultationTickets.find((t) => t.id === ticketId || t.ticketId === ticketId);
  if (ticket) {
    ticket.status = 'Completed';
    ticket.specialistNotes = notes;
    return ticket;
  }
  return null;
}
