...
const defaultProfile: ProfileState = {
  ...
  bio: 'Exploring self-growth, relationships, and spiritual guidance through reflective readings.',
  spiritualInterests: ['Tarot', 'Palmistry', 'Self Growth'],
  primaryLifeGoal: 'Personal Growth',
  secondaryGoals: ['Career Growth', 'Relationships'],
  guidanceAreas: ['Relationships', 'Career', 'Personal Growth'],
  readingDepth: 'deep',
  readingStyle: 'detailed',
  preferredTopics: ['Personality', 'Relationships', 'Career', 'Personal Growth'],
  dailyGuidance: true,
...
export default function ProfilePreviewPage() {
  ...
  const zodiacSign = useMemo(() => getZodiacSign(profile.birthDate), [profile.birthDate]);
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-8 flex items-center justify-between gap-3">
          ...
          <Button variant="outline" size="sm" className="border-primary/40 bg-background/70 text-foreground hover:border-primary/60 hover:bg-primary/5">
        ...
        <div className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <input
                    value={zodiacSign}
                    readOnly
                    className="w-full rounded-2xl border border-border/70 bg-background/80 px-3 py-2.5 text-sm text-foreground outline-none"
                  />
                </div>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-border/60 pt-5 sm:flex-row sm:justify-end">
            <Button type="button" variant="outline" onClick={handleCancel} className="border-border/70 bg-background/60 text-foreground hover:bg-primary/5">
              Cancel
            </Button>
            <Button type="button" onClick={handleSave} className="bg-primary text-primary-foreground shadow-[0_0_25px_rgba(244,196,94,0.25)] hover:bg-primary/90">
              {isSaved ? <><Check className="mr-2 h-4 w-4" /> Saved</> : 'Save Changes'}
            </Button>
          </div>
        </div>
      </div>
